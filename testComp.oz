...
Display = component(inPorts:'in'(input:port(q:{NewQueue} % The buffer of the port, will be hidden for the user
					   p:proc {$ IP Out Var} {Browse 1#IP} end % The procedure to deal with the IP
...
Display2 = component(inPorts:'in'(input:port(q:{NewQueue}
					   p:proc {$ IP Out Var} {Browse 2#IP} end
					   s:nil
					  )
			       )
		    outPorts:out
...
Mul = component(inPorts:'in'(x:port(q:{NewQueue}
				    p:proc{$ IP Out Var} {Browse begin} Var.x = IP end
				    s:nil)
			     y:port(q:{NewQueue}
				    p:proc{$ IP Out Var} Var.y = IP end
				    s:nil)
			     z:arrayPort(qs:queues()
					 p:proc{$ IPs Out Var} Var.z = {FoldR IPs fun{$ E Acc} E+Acc end 0} end
					 s:[nil])
			    )
		outPorts:out(output:nil)
		procs:procs(proc {$ Out Var}
			       {Browse beginOut}

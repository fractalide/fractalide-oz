...
declare
Even = {NewComponent [input] %Input ports : input
	nil % No output ports
	proc{$ In Out} {Browse In.input # ' is an even Number'} end % The proc to apply
       }
Odd = {NewComponent [input]
       nil
       proc{$ In Out} {Browse In.input # ' is an odd number'} end
      }
Filter = {NewComponent [input]
	  [even odd]
	  proc{$ In Out}
	     case In.input mod 2
	     of 0 then {Send Out.even In.input}
	     else {Send Out.odd In.input}
	     end
	  end }
%Bind componenet together
P1 = Filter.outPorts.even
P1 := Even.inPorts.input
P2 = Filter.outPorts.odd
P2 := Odd.inPorts.input

{Send Filter.inPorts.input 666}
{Send Filter.inPorts.input 11}
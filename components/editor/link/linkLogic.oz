...
define
   ...
   proc{InputProc In Out Comp} IP in
	 ...
	 {Comp.state.bPoint bind(action#moveBegin Comp.state.subComp 'in')}
	 {Comp.state.bPoint bind(action#moveBeginPos Comp.state.subComp 'in')}
	 {Comp.state.bPoint bind(action#delete Comp.state.subComp 'in')}
	 ...
	 {IP.ePoint bind(action#moveEnd Comp.state.subComp 'in')}
	 {IP.ePoint bind(action#moveEndPos Comp.state.subComp 'in')}
	 {IP.ePoint bind(action#delete Comp.state.subComp 'in')}
	 ...
	 {Comp.state.bPoint send('in' openPorts Ack)}
	 {Wait Ack}
	 {Comp.state.ePoint send('in' openPorts Ack2)}
	 {Wait Ack2}
      [] close andthen {HasFeature IP comp} then Ack Ack2 in
	 {Comp.state.bPoint send('in' closePorts(comp:IP.comp) Ack)}
	 {Wait Ack}
	 {Comp.state.ePoint send('in' closePorts(comp:IP.comp) Ack2)}
	 {Wait Ack2}
      [] close then Ack Ack2 in
	 {Comp.state.bPoint send('in' closePorts Ack)}
	 {Wait Ack}
	 {Comp.state.ePoint send('in' closePorts Ack2)}
	 {Wait Ack2}
      end
   end
end